...
- **Coding standards (mandatory)** — `docs/CODING_STANDARDS.md` is authoritative: function body ≤ 20 lines, file ≤ 150 lines, ≤ 2 indentation levels past the body, cyclomatic complexity ≤ 8, ≤ 4 params. Deno lint runs the recommended tag as errors; `no-undef` is off (browser/Node globals).
- **Reviewer gate (mandatory)** — after EVERY task, a fresh secondary reviewer agent runs an independent pass checking: size limits, code smells (long functions, deep nesting, duplication, premature abstraction, naming drift, dead code), FSD layer rules, Tell-Don't-Ask, SOLID, error-model compliance, and test coverage depth (all five categories). Blocking findings must be fixed before the task is accepted; non-blocking findings are logged as follow-ups. The reviewer never fixes — it reports.
- **PR granularity** — work ships as feature-sized PRs (~300 changed lines, ~6 files max, one concern each). Oversized work is split before review. Each PR passes `deno task verify` before review.
