...
server.listen(APPSERVERPORT, ()=>{
    console.log(`Remix HTTPS server listening on ${process.env.NODE_ENV === 'production'?'https':'http'}://localhost:${APPSERVERPORT}...`);
});
...
const origin = `${process.env.NODE_ENV === 'production'?'https':'http'}://localhost:${APPSERVERPORT}`;
