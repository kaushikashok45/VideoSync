...
import { PlayIcon, PauseIcon, ForwardIcon, BackwardIcon, ArrowsPointingOutIcon, ArrowsPointingInIcon, SpeakerWaveIcon, SpeakerXMarkIcon } from '@heroicons/react/24/solid';
import {useEffect, useRef, useState, forwardRef, useImperativeHandle, MutableRefObject} from "react";
import {peerVideoMeta} from "~/utils/peerSignalContract";
...
const VideoPlayer = forwardRef(({videoURL}: videoURLProps,ref) => {
    ...
    useImperativeHandle(ref, () => ({
        setVideoMeta: (videoMeta: peerVideoMeta) => {
            ...
            pauseDelay.current = (videoRef.current)?(videoRef.current?.currentTime - (videoMeta.currentTime + 0.5)):0;
        ...
        addEventListener(eventName:string,script:EventListenerOrEventListenerObject){
            videoRef.current?.addEventListener(eventName,script);
        },
        removeEventListener(eventName:string,script:EventListenerOrEventListenerObject){
    ...
    const playVideoInFullScreen = (event:React.MouseEvent<HTMLButtonElement>) => {
        event.stopPropagation();
    ...
    const exitVideoFromFullScreen = (event:React.MouseEvent<HTMLButtonElement>) => {
        event.stopPropagation();
        if (isFullScreen) {
    ...
    const preventClickEventPropagation = (event:React.MouseEvent<HTMLDivElement>) => {
        event.stopPropagation();
    }
    ...
    return(
        <div id="videoContainer" className="relative w-[70%] max-w-[800px]" onMouseEnter={handleMouseEnter} onBlur={handleMouseLeave} onMouseLeave={handleMouseLeave} onClick={handlePlay} onMouseMove={handleMouseMove}>
            <video className={`videoPlayer w-full block rounded-lg ${isFullScreen?'h-full':''}`} src={videoURL} ref={combinedRef} controls={false} autoPlay playsInline></video>
            <div className={`controls w-full absolute ${isFullScreen?'bottom-4 py-4':'bottom-0 py-4'} left-1/2 transform -translate-x-1/2 flex-col items-center bg-black/20 backdrop-blur-md backdrop-saturate-150 border border-white/20 px-4 rounded-lg transition-opacity duration-300 ease-in-out ${canShowControls ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                ...
                <div className={`playbackControls absolute left-1/2 bottom-0 transform -translate-x-1/2  flex flex-row gap-5 ${isFullScreen?'mb-10':'py-1'}`} onClick={preventClickEventPropagation}>
            ...
            <div className={`playbackControls p-1.5 absolute bg-black/20 backdrop-blur-md backdrop-saturate-150 border border-white/20 px-4 rounded-lg transition-opacity duration-300 ease-in-out ${isFullScreen?'top-10 right-10':'top-5 right-5'} ${canShowControls ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                {!isFullScreen ? (
                    <button className="controlButton mr-auto" onClick={playVideoInFullScreen}>
                        <ArrowsPointingOutIcon className="h-5 w-6 text-white" />
                    </button>
                ):(
                    <button className="controlButton mr-auto" onClick={exitVideoFromFullScreen}>
                        <ArrowsPointingInIcon className="h-5 w-6 text-white" />
                    </button>
                )}
            </div>
            <div className={`playbackControls p-1.5 absolute bg-black/20 backdrop-blur-md backdrop-saturate-150 border border-white/20 px-4 rounded-lg transition-opacity duration-300 ease-in-out ${isFullScreen?'top-10 left-10':'top-5 left-5'} ${canShowControls ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                {isMuted ? (
